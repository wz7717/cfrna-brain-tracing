...
from scripts.build_bo2023_reference_projector import DEFAULT_COUNTS, DEFAULT_VSD  # noqa: E402
...
def read_labels(path: Path, sheet: str, network_col: str) -> pd.Series:
    info = pd.read_excel(path, sheet_name=sheet, usecols=["No.", "Region", network_col])
    ...
    return info.drop_duplicates("sample_id").set_index("sample_id")["network_id"]

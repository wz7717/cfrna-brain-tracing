...
EXACT_STAGED_SHA256 = "401441CFD7FF9B66408377CD854CF8A4C31B869F16C6D843E35DFCA63BE401C1"
EXACT_ORIGIN_LOCATOR = "external_source::formal_lomo_exact_origin/formal_lomo_exact_region_detail.csv"
...
def test_lomo_exact_derived_artifacts_share_current_source() -> None:
    ...
    assert provenance["formal_source"]["staged_sha256"] == sha256_utf8_lf_text(
        CANONICAL_FORMAL_PATH
    )
...
def test_lomo_exact_origin_staged_and_generator_input_are_explicitly_paired() -> None:
    ...
    assert origin["path"] == EXACT_ORIGIN_LOCATOR
    assert staged["path"] == CANONICAL_FORMAL_PATH.relative_to(ROOT).as_posix()
    assert staged["sha256"] == EXACT_STAGED_SHA256
    assert staged["sha256"] == sha256_utf8_lf_text(CANONICAL_FORMAL_PATH)
    ...
    assert formal_source["path"] == staged["path"]
    assert formal_source["sha256"] == staged["sha256"]
    assert formal_source["origin_path"] == origin["path"]
    assert formal_source["origin_sha256"] == origin["sha256"]
    assert origin["sha256"] != staged["sha256"]
    ...
    macro_provenance = macro["provenance"]
    assert macro_provenance["formal_lomo_exact_source_sha256"] == staged["sha256"]
    assert macro_provenance["formal_lomo_exact_origin_sha256"] == origin["sha256"]
